...
class AnalyseController extends Controller
{
    ...
    // POST: dispatch a sentiment analysis. Returns 202 if queued or already
    // running; the frontend polls sentimentStatus() for the result.
    public function sentiment(int $postId)
    {
        ...
        if (($intel['sentiment_pending'] ?? false) === true) {
            return response()->json(['status' => 'pending'], 202);
        }

        $intel['sentiment_pending'] = true;
        unset($intel['sentiment_error']);
        $post->intelligence = $intel;
        $post->save();

        \App\Jobs\AnalyseSentimentJob::dispatch($post->id);

        return response()->json(['status' => 'pending'], 202);
    }

    // GET: client poll for sentiment job status / latest result.
    public function sentimentStatus(int $postId)
    {
        $post = Post::findOrFail($postId);
        $intel = $post->intelligence ?? [];
        ...
        if (!empty($intel['sentiment_error'])) {
            return response()->json(['status' => 'failed', 'error' => $intel['sentiment_error']]);
        }
        if (!empty($intel['sentiment'])) {
            return response()->json(['status' => 'complete'] + $intel['sentiment']);
        }
